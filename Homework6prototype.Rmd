...
for (i in 1:length(seq_5)) {
  ...
  if (i > 1) {
    if (flu_data_sum[[1]]$`Pr(>F)`[1] < 0.05) {
      count = count + 1
     cat("There is a signifact difference between colder and warmer region flu cases at a stdev value of ", seq_5[i], "with a p-value of ", flu_data_sum[[1]]$`Pr(>F)`[1], "\n")
      }
    
  }
  cat("Total signifcant flu cases ", count, "out of ", i, "trials", "\n") 
  cat("Smallest standard deviation size for significant difference", smallest_mean_size, "\n")
}
